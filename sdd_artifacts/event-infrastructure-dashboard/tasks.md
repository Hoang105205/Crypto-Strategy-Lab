...
**Purpose**: Reconcile authoritative contracts before code and establish the shared types, public ports, persistence fields, and frontend test harness required by every subfeature.

- [ ] **T001** [Foundation] Reconcile `kb/contracts/events.yaml`, `kb/modules/event-infrastructure.md`, `kb/MODULES.md`, and `kb/flows/strategy-backtest.md` with plan gate P0: define three total attempts with retry delays `[1000, 4000]`, `USER`-before-`SEARCH_LOOP` priority with FIFO inside each source, producer-owned `jobId`, terminal-only `BacktestFailed` without `willRetry`, canonical `/strategies`, `getCandlesRange`, and `run`, and Strategy-owned execution/result ports; preserve all existing active event payload fields. This task blocks T002-T049.
...
- [ ] **T006** [Foundation] Run shared type-check/build and Prisma validation against `workspace/libs/shared/package.json` and `workspace/apps/backend/prisma/schema.prisma`, generate the Prisma client, and add contract-compile assertions in `workspace/libs/shared/src/interfaces/infrastructure.spec.ts` for required `jobId`, terminal failure shape, Strategy ports, and queue/Loop enums. Depends on T002-T005; this is the P0 gate for all story phases.
...
**Goal**: Accept producer-generated backtest jobs, schedule USER work first, execute at concurrency three, retry deterministically, persist results through ports, and dead-letter terminal failures exactly once.

**Independent test**: Enqueue USER and SEARCH_LOOP jobs, observe priority/FIFO and max concurrency 3, complete one job, retry another at 1s/4s, and inspect/retry one terminal dead-letter job through REST.

- [ ] **T012** [US2] Write failing queue tests in `workspace/apps/backend/src/queue/backtest.queue.spec.ts` for payload validation, duplicate `jobId` conflict, two-source priority/FIFO, lifecycle states, concurrency cap 3, queue stats, injectable scheduling, retry re-entry, and manual dead-letter retry. Depends on T011 and `contracts/job-queue-worker.md`.
- [ ] **T013** [P] [US2] Write failing worker and dead-letter tests in `workspace/apps/backend/src/queue/backtest.worker.spec.ts` and `workspace/apps/backend/src/queue/dead-letter.repository.spec.ts` using fake clocks, fake Strategy/Market Data/result ports, and mocked Prisma: success ordering, zero-candle/unknown-version non-retryability, 1s then 4s delays, three total attempts, result-before-event persistence, and exactly-once `BacktestFailed` plus `BacktestDeadLettered`. Depends on T011.
- [ ] **T014** [US2] Create queue policy/config and errors in `workspace/apps/backend/src/queue/queue.config.ts`, `workspace/apps/backend/src/queue/queue-scheduler.ts`, and `workspace/apps/backend/src/queue/queue.errors.ts`, reading `BACKTEST_WORKER_CONCURRENCY=3` and `BACKTEST_MAX_ATTEMPTS=3` with validated defaults and exposing an injectable clock/scheduler for fake-timer tests. Depends on T012-T013.
- [ ] **T015** [US2] Implement `workspace/apps/backend/src/queue/backtest.queue.ts` with separate USER and SEARCH_LOOP FIFO buffers, a job registry, bounded dispatch, duplicate protection, status/stat snapshots, retry scheduling, and preserved producer `jobId`/correlation identity; make T012 pass. Depends on T014.
...
- [ ] **T019** [US2] Wire event subscription, queue, worker pool, repositories, controllers, config, and shared tokens in `workspace/apps/backend/src/queue/queue.module.ts` and import it from `workspace/apps/backend/src/app.module.ts`; add zero-argument provider factories where runtime configuration is needed and verify module boot in `workspace/apps/backend/src/queue/queue.module.spec.ts`. Depends on T015-T018.
- [ ] **T020** [US2] Add end-to-end queue scenarios in `workspace/apps/backend/src/queue/queue.integration.spec.ts` covering USER priority, three parallel workers, a successful persisted result, retry exhaustion/dead-letter, no duplicate terminal Events, and REST inspection/retry; run targeted tests/type-check and append the US2 evidence to `sdd_artifacts/event-infrastructure-dashboard/validation.md`. Depends on T019.

**Checkpoint**: US2 works with fakes independently of unfinished Strategy Engine implementations and exposes a documented queue-backend replacement seam.
...
- [ ] **T048** [Foundation] Execute every scenario in `sdd_artifacts/event-infrastructure-dashboard/quickstart.md` with test doubles where teammate implementations are unavailable, verify all acceptance criteria and success criteria SC-001-SC-015 from `sdd_artifacts/event-infrastructure-dashboard/spec.md`, and record evidence/gaps in `sdd_artifacts/event-infrastructure-dashboard/validation.md`. Depends on T046-T047.
- [ ] **T049** [Foundation] Update `README.md`, `workspace/apps/backend/.env.example`, `workspace/apps/frontend/.env.example`, `kb/modules/event-infrastructure.md`, and `sdd_artifacts/event-infrastructure-dashboard/quickstart.md` with startup/configuration, namespace/channel details, stable errors, known in-memory restart limitation, and demonstrations for swapping Event Bus adapter, queue backend, scoring policy, and Strategy generator without consumer/orchestrator changes. Depends on T048.
