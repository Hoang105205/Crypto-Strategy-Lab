...
4. **LoopController requests a candidate** — LoopController gọi Facade `SearchEngine` của Strategy Engine (truyền vào `generatorType`) để lấy candidate thay vì gọi trực tiếp từng `IStrategyGenerator`.
