...
| App-level Leaderboard Live Provider (Frontend dependency) | Mounted below `AuthProvider` and `InfrastructureProvider`; owns cross-route Live updates preference/cache, current-session REST reconciliation, disconnected-only 15-second fallback, identity-generation invalidation, and exactly one `leaderboard:update` handler | Provider / Safe Invalidation Consumer | Frontend app root/provider layer |
...
- **Trade-offs**:
  ...
  - Negative: eventual consistency — there's a small window between `BacktestCompleted` and the Leaderboard reflecting it. The frontend normally receives `LeaderboardUpdated` over WebSocket; when Live is ON and that socket is disconnected, a bounded 15-second REST fallback limits staleness until reconnection.
...
- **How**: On startup, `LeaderboardModule` calls `eventBus.subscribe('BacktestCompleted', handler)`. The handler is idempotent on `backtestResultId`, computes `score`, and inserts one nullable-owner entry with non-authoritative stored `rank = 0`. `strategyVersionId` and `backtestResultId` are logical ID references rather than database foreign keys. A startup and five-minute reconciler validates them through Strategy Engine's public `IBacktestResultPort`, deletes only confirmed missing/mismatched projections, and keeps rows when the port check fails transiently. It never rewrites surviving ranks. Caller-scoped REST reads exclude invalid sources, apply visibility, select the best result per strategy version, and compute contiguous rank/Top-K at read time; the handler publishes `LeaderboardUpdated` with only the system Top-K so the namespace-wide payload is a safe invalidation.
- **Trade-offs**:
...
Strategy Engine (source=USER) / Loop Controller (source=SEARCH_LOOP)
   ...
   │  2. insert rank=0       │        │     current loopRunId      │
   │  3. read system Top-K   │        │  2. check stop conditions  │
...
sequenceDiagram
    ...
    LB->>LB: compute score, insert rank=0, read bounded system Top-K
...
|--------|--------|---------------|
| LeaderboardEntry | `id (UUID, PK)`, `userId (UUID, nullable; null = system)`, `strategyVersionId (UUID, logical reference)`, `backtestResultId (UUID, UNIQUE, logical reference)`, `rank (int, stored as non-authoritative 0)`, `score (float)`, `totalReturn (float)`, `winRate (float)`, `maxDrawdown (float)`, `sharpeRatio (float)`, `totalTrades (int)`, `createdAt`, `updatedAt` | Denormalized Event Infrastructure read model. Both source IDs refer logically to Strategy Engine records but intentionally have no database FK/relation. Source validity is reconciled through `IBacktestResultPort`; confirmed orphans are deleted without updating survivors, and contiguous rank is assigned only in read projections. |
...
- **Performance**: BullMQ worker concurrency is configurable (default 3). USER priority prevents the global search loop from indefinitely delaying interactive work. A leaderboard result performs one insert rather than insert plus N rank updates. Scoped PostgreSQL ranking/index queries bound work before Top-K projection; an event causes at most one app-provider reconciliation path rather than one handler per mounted route. The disconnected fallback adds at most one 15-second provider reconciliation loop while Live is ON.
...
- **Observability**: `correlationId` propagates through `BacktestRequested → BacktestCompleted/Failed → LeaderboardUpdated` and through `SearchLoopStarted → BacktestRequested → SearchLoopProgress → SearchLoopStopped`, enabling full request tracing in logs. `GET /api/queue/stats` and `GET /api/dashboard/summary` retain live queue/loop counts for backend diagnostics or future operator tooling; the end-user Dashboard intentionally renders only compact Search Loop lifecycle status and the leaderboard Live-update switch.
...
- **Unit tests**:
  ...
  - `LeaderboardService`: score computation is correct for known inputs; duplicate `BacktestCompleted` (same `backtestResultId`) does not create a duplicate entry; one insert stores non-authoritative rank 0; read-time Top-K keeps exactly K entries sorted/ranked correctly; cleanup removes confirmed orphan/mismatched references, skips transient lookup failures, deduplicates overlapping runs, and never updates survivors.
  ...
  - App-level provider: default OFF without stored choice; explicit ON/OFF persistence across remount/reload; one handler while ON; one 15-second REST interval only while ON and disconnected; OFF snapshot freeze across navigation/reload/reconnect; exact handler/interval cleanup; and A → B/A → anonymous cache/request invalidation before render.
...
- **Integration tests**:
  ...
  - Leaderboard reconciliation: remove a source `BacktestResult`, invoke cleanup, and assert its projection alone is removed while surviving rows are not updated; the next read returns contiguous ranks without accessing another module's Prisma delegates.
