...
- **Source files**:
  ...
  - Frontend: `apps/frontend/src/app/strategy/`, `apps/frontend/src/components/strategy/` (canonical user route: `/strategy`; `/strategies` is an unused scaffold)
...
| MovingAverageStrategy | Moving Average crossover signal (fast/slow period) | Strategy | `strategy/strategies/moving-average.strategy.ts` |
| RSIStrategy | RSI overbought/oversold signal (period, thresholds) | Strategy | `strategy/strategies/rsi.strategy.ts` |
| BollingerBandsStrategy | Bollinger Bands breakout signal (period, stdDev) | Strategy | `strategy/strategies/bollinger-bands.strategy.ts` |
...
| MacdStrategy | MACD crossover signal | Strategy | `strategy/strategies/macd.strategy.ts` |
| AtrStrategy | ATR volatility/breakout signal | Strategy | `strategy/strategies/atr.strategy.ts` |
| StochasticStrategy | Stochastic K/D crossover signal | Strategy | `strategy/strategies/stochastic.strategy.ts` |
| CompositeStrategy | Combines N child strategies via a combiner | Composite | `strategy/composite/composite.strategy.ts` |
| MajorityVoteCombiner | Majority-vote signal aggregation | Composite Combiner | `strategy/combiners/majority-vote.combiner.ts` |
| WeightedScoreCombiner | Weighted-score signal aggregation | Composite Combiner | `strategy/combiners/weighted-score.combiner.ts` |
| BacktesterService | Replay historical candles through an isolated incremental session when available, then simulate trades | Execution | `strategy/backtester/backtester.service.ts` |
| EvaluatorService | Compute Return, WinRate, MDD, Sharpe, ProfitFactor from trades | Evaluation | `strategy/evaluator/evaluator.service.ts` |
...
| StrategyVersioningService | Create, retrieve, compare strategy version snapshots | Versioning | `strategy/versioning/strategy-versioning.service.ts` |
| StrategyController | REST endpoints for strategy reads, composite creation, backtest submission, and version listing | Controller | `strategy/controllers/strategy.controller.ts` |
...
- **How**: All strategies implement the `IStrategy` interface. `StrategyRegistry` maintains a `Map<string, IStrategy>`. Registration validates name uniqueness. The registry exposes `analyze(name, candles)` which delegates to the registered strategy. Built-ins additionally implement optional `createAnalysisSession()` so one isolated session can advance indicators with `next(candle)` during a backtest; `analyze(candles)` remains the compatibility and direct-analysis contract.
- **Trade-offs**: 
...
- **How**: `CompositeStrategy` holds an array of child `IStrategy` instances + an `ICombiner`. Direct `analyze(candles)` runs each child and combines its signals. During backtesting, `createAnalysisSession()` creates one isolated child session per child, advances each session with the same candle, and combines the resulting signals. Children without an incremental session retain the compatible accumulated-prefix fallback. `MajorityVoteCombiner` counts signal types and returns the majority. `WeightedScoreCombiner` assigns numeric weights (+1 BUY, -1 SELL, 0 HOLD), computes weighted sum, and thresholds.
- **Trade-offs**: 
  - ✅ Recursive composition — a composite can contain other composites
  - ✅ Combiner is pluggable — new combiners implement `ICombiner`
  - Trade-off: composite work is proportional to the number of children per candle; compatible plugins without an incremental session may still recompute over their accumulated prefix.
...
- **Security**: `SupabaseJwtGuard` verifies optional identity for all Strategy endpoints so reads can be scoped to system plus current-user data. `POST /api/strategies/composite`, `POST /api/strategies/backtest`, and `GET /api/strategies/backtest/:id` additionally require `RequireAuth`; anonymous callers receive 401. Backtest-result reads filter by the verified `userId`. Input validation covers strategy parameters, and Prisma parameterization prevents SQL injection.
- **Performance**: 
  - Backtesting is CPU-bound — offloaded to Job Queue workers (Phương's infrastructure).
  - Built-in strategies create isolated incremental analysis sessions, so a single strategy processes M candles in O(M) without copying or recalculating every growing prefix.
  - A composite with N incremental children processes M candles in O(N × M); child session calls for the same candle are awaited together before combining signals.
  - Third-party strategies may omit `createAnalysisSession()` for compatibility, but their accumulated-prefix `analyze`/`analyzeAsync` implementation is responsible for avoiding expensive full-history recomputation.
...
- **Unit tests**:
  ...
  - Every built-in incremental session produces the same candle-by-candle signals as prefix-based `analyze()` and keeps state isolated between backtests.
...
- **Integration tests**:
  ...
  - Authenticated `POST /api/strategies/backtest` → awaits mock `IJobQueue.enqueue`, publishes `BacktestRequested` only on success, and returns `503 QUEUE_UNAVAILABLE` on enqueue failure; anonymous access returns 401.
