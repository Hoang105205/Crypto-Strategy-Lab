...
| `kb/contracts/` | **Each module owner** writes their own, **Hoàng reviews** | `market-data.yaml` → Hoàng, `strategy.yaml` → Huy, `news.yaml` → Thuận, `events.yaml` → Phương (all use `.yaml` extension for structured contracts) |
...
| **Wed** | Write ADR-001 (Modular Monolith), ADR-002 (Plugin Architecture), ADR-003 (Event-Driven), ADR-004 (Adapter Pattern). Define shared event type interfaces. | Implement `MAStrategy` (first real strategy). Prove Registry + analyze() pipeline end-to-end with mock data. | Implement first `INewsProvider` (RSS adapter). Prove collection → normalization → storage pipeline with mock data. | Implement `CandlestickChart` component with lightweight-charts and mock candle data. Implement EventEmitter2 wrapper. Define all event types (`MarketDataUpdated`, `BacktestRequested`, `BacktestCompleted`, `LeaderboardUpdated`). Write `events.yaml`. |
| **Thu** | Implement `BinanceAdapter` (historical API first). `MarketDataService` with caching. Prisma migrations. Write `market-data.yaml`. | Implement `RSIStrategy`, `BollingerBandsStrategy`, `SupportResistanceStrategy`. Write `strategy.yaml`. | Implement Python sentiment service (VADER). Wire `SentimentClient` in NestJS. Prove: RSS news → analyze → score. Write `news.yaml`. | Implement `MultiTimeframeGrid` — 4 charts, independent timeframes. Real-time chart updates with mock WebSocket. **Scaffold Job Queue**: in-memory queue with worker stub, `IJobQueue` interface. Write DESIGN.md. |
