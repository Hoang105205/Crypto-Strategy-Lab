...
export class RsiStrategy implements IStrategy, OnModuleInit {
  ...
  analyze(candles: Candle[]): Signal {
    ...
    const rsiValues = RSI.calculate({ period: this.period, values: closePrices });
    ...
    const previousRsi = rsiValues.length > 1 ? rsiValues[rsiValues.length - 2] : latestRsi;
    ...
    if (latestRsi >= this.overbought) {
      return { action: SignalAction.SELL, confidence: 0.6, metadata: { rsi: latestRsi } };
    }
    if (latestRsi <= this.oversold) {
      return { action: SignalAction.BUY, confidence: 0.6, metadata: { rsi: latestRsi } };
    }
